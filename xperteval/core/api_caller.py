...
def invoke_model_api(model_config: Dict[str, Any], request_payload: Dict[str, Any], 
    ...
    while attempt <= max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 429:  # 速率限制
                    ...
                    if attempt < max_retries:
                        logger.info(f"触发速率限制，将在{retry_after}秒后重试")
                        time.sleep(retry_after)
                        attempt += 1
                        continue
                    else:
                        raise ApiError(
                            message="API速率限制，已达到最大重试次数",
                            status_code=response.status_code, 
                            details=error_details
                        )
                elif 500 <= response.status_code < 600:  # 服务器错误
                    if attempt < max_retries:
                        logger.info(f"服务器错误，{retry_delay}秒后重试")
                        time.sleep(retry_delay)
                        attempt += 1
                        continue
                    else:
                        raise ApiError(
                            message="API服务器错误，已达到最大重试次数", 
                            status_code=response.status_code, 
                            details=error_details
                        )
                else:  # 其他错误（如认证错误、参数错误等）
                    raise ApiError(
                        message=f"API请求失败: {error_details.get('error', {}).get('message', '未知错误')}", 
                        status_code=response.status_code, 
                        details=error_details
                    )
            ...
            if isinstance(e, (requests.Timeout, requests.ConnectionError)) and attempt < max_retries:
                logger.info(f"连接异常，{retry_delay}秒后重试")
                time.sleep(retry_delay)
                attempt += 1
                continue
            else:
    ...
    # 如果到达这里，说明已经达到最大重试次数
    if last_error:
        raise ApiError(f"API请求失败，已达到最大重试次数: {str(last_error)}")
