...
def scan_integrated_datasets(base_dir: Optional[Union[str, Path]] = None) -> Dict[str, Dict[str, Any]]:
    ...
    for dataset_dir in Path(base_dir).glob("*"):
        ...
        for file_pattern in ["*.jsonl", "*.json"]:
            matches = list(dataset_dir.glob(file_pattern))
            if matches:
                # 优先选择"dataset.jsonl"文件
                for match in matches:
                    if match.name == "dataset.jsonl":
                        dataset_file = match
                        break
                
                # 如果没有找到特定名称的文件，使用第一个匹配
                if not dataset_file:
        ...
        if dataset_file:
            datasets[dataset_id] = {
                ...
                "media_dir": str(dataset_dir / "media") if (dataset_dir / "media").exists() else None
            }
